...
const Sidebar = ({ role }) => {
  const links = role === 'admin' ? [
    ...
    { name: 'Admin Analytics', path: '/admin/analytics' },
    { name: 'Power BI', path: '/user' },
    { name: 'Profile', path: '/admin/users' },
    { name: 'Total Earnings', path: '/admin/totalearning' },
    { name: 'Special Offers', path: '/admin/SpecialOffers' },
    { name: 'Settings', path: '/user/settings' }
  ];
        ...
        <ul>
          {links.map((link, index) => (
            ...
            >
              <Link
                to={link.path}
                className="block text-lg hover:text-gray-300 transition-colors duration-300 ease-in-out"
              >
                {link.name}
              </Link>
            </motion.li>
